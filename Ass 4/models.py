...
# First variant model, with weight-decay activation
...
# Second variant model, with weight-decay and ReLU activation
...
# Third variant model, with weight-decay, ReLU activation and different kernel size
class LeNet5Variant3(nn.Module):
    def __init__(self):
        ...
        # Fully connected layers
        self.fc1 = nn.Linear(16 * 6 * 6, 120)  # 4x4 image dimension after 2x2 pooling twice
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 10)  # 10 output classes for Fashion-MNIST
...
# Fourth variant model, with weight-decay, ReLU activation, different kernel size and dropout
...
# Fourth variant model, with ReLU activation, different kernel size, one more convolutional layer and dropout
class LeNet5_best_multiple_outputs(nn.Module):
    def __init__(self):
        super(LeNet5_best_multiple_outputs, self).__init__()
        # First convolutional layer
        self.conv1 = nn.Conv2d(1, 12, kernel_size=3, padding=1)
        # Second convolutional layer
        self.conv2 = nn.Conv2d(12, 32, kernel_size=3)
        # Fully connected layers
        self.fc1 = nn.Linear(32 * 6 * 6, 120)  # 4x4 image dimension after 2x2 pooling twice
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 10)  # 10 output classes for Fashion-MNIST
        # Dropout layer
        self.dropout = nn.Dropout(p=0.2)
    ...
    def forward(self, x):
        # First convolutional layer with ReLU activation and 2x2 max pooling
        x = F.relu(self.conv1(x))
        feature_maps_conv1 = x.detach().cpu().numpy()
        x = F.relu(F.max_pool2d(x, 2, stride=2))
        # Second convolutional layer with ReLU activation and 2x2 max pooling
        x = self.conv2(x)
        x = F.relu(F.max_pool2d(x, 2, stride=2))
