...
from models import Stanford40_model, HMDB51_model, Conv2Plus1Model, HMDB51_Fusion_Model
...
# HMDB51 - OPTICAL FLOW
if not os.path.exists(f'Conv2Plus1Model_dynamic.pth'):
    # HMDB51 - Optical Flow
    keep_hmdb51 = ["clap", "climb", "drink", "jump", "pour", "ride_bike", "ride_horse", 
                "run", "shoot_bow", "smoke", "throw", "wave"]
    ...
    # Create custom dataset
    train_dataset = HMDB51_OF_Dataset(train_files, train_labels, root_dir="video_OF_py_dataset")
    val_dataset = HMDB51_OF_Dataset(val_files, val_labels, root_dir="video_OF_py_dataset")
    test_dataset = HMDB51_OF_Dataset(test_files, test_labels, root_dir="video_OF_py_dataset")
...
'''if os.path.exists(f'Conv2Plus1Model_dynamic.pth'):
    ...
    # Initialize model, loss function, and optimizer
    model = Conv2Plus1Model()
    model.load_state_dict(torch.load(f'Conv2Plus1Model_dynamic.pth'))
    test_accuracy, confusion_matrix = test_model(model, test_loader, device='cpu')
    print(f'Test accuracy: {test_accuracy:.2f}% for HMDB51_model_cyclic.pth')'''

# HMDB51 - OPTICAL FLOW
if not os.path.exists(f'HMDB51_Fusion_Model_dynamic.pth'):
    # HMDB51 - Optical Flow
    keep_hmdb51 = ["clap", "climb", "drink", "jump", "pour", "ride_bike", "ride_horse", 
                "run", "shoot_bow", "smoke", "throw", "wave"]
    all_train_files, all_train_labels, test_files, test_labels = create_hmdb51_splits(keep_hmdb51)
    ...
    # Create custom dataset
    train_dataset = HMDB51_Fusion_Dataset(train_files, train_labels, frames_root_dir="video_image_dataset", frame_number=50, optical_flow_root_dir="video_OF_py_dataset", transform=ToTensor())
    val_dataset = HMDB51_Fusion_Dataset(val_files, val_labels, frames_root_dir="video_image_dataset", frame_number=50, optical_flow_root_dir="video_OF_py_dataset", transform=ToTensor())
    test_dataset = HMDB51_Fusion_Dataset(test_files, test_labels, frames_root_dir="video_image_dataset", frame_number=50, optical_flow_root_dir="video_OF_py_dataset", transform=ToTensor())
...
if os.path.exists(f'Conv2Plus1Model_dynamic.pth'):
    ...
    # Initialize model, loss function, and optimizer
    model = HMDB51_Fusion_Model()
    model.load_state_dict(torch.load(f'HMDB51_Fusion_Model_epoch_1_dynamic.pth'))
    test_accuracy, confusion_matrix = test_model(model, test_loader, device='cpu')
    print(f'Test accuracy: {test_accuracy:.2f}% for HMDB51_Fusion_Model_dynamic.pth')